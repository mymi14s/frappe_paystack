frappe.ui.form.on('Sales Order', {
  refresh(frm) {
    ...
    const advance_paid = flt(frm.doc.advance_paid || 0);
    const so_outstanding = Math.max(0, grand_total - advance_paid);
    const can_pay = frm.doc.docstatus === 1 && so_outstanding > 0 &&
    ...
    frappe.call({
      ...
      if (can_pay) {
        ...
        frm.add_custom_button(__('Pay via Paystack (SO)'), () => {
          make_paystack_link(frm, {
            doctype: frm.doc.doctype,
            docname: frm.doc.name,
            amount: so_outstanding,
            currency: frm.doc.currency || 'NGN',
          });
        }, __('Paystack'));

        frm.add_custom_button(__('Send Paystack Link (SO)'), () => {
        ...
        frm.add_custom_button(__('Partial Payment'), () => {
          ...
          primary_action(values) {
            if (values.amount > 0 && values.amount <= so_outstanding) {
              ...
              frappe.throw("Amount must be > 0 or <= grand_total");
            }
          }
        });
        partial_dialog.show();
