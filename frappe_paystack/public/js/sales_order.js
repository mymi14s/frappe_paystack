frappe.ui.form.on("Sales Order", {
	refresh(frm) {
		...
		const grandTotal = flt(frm.doc.grand_total || 0);
		const advancePaid = flt(frm.doc.advance_paid || 0);
		const soOutstanding = Math.max(0, grandTotal - advancePaid);
		const canPay =
			frm.doc.docstatus === 1 &&
			soOutstanding > 0 &&
			...
			frm.add_custom_button(__("Partial Payment"), () => {
				const partialDialog = new frappe.ui.Dialog({
					...
					primary_action(values) {
						if (values.amount > 0 && values.amount <= soOutstanding) {
